//! The `/herdr` command: runs the configured `herdr.control_command` as a
...
///
/// The command is the configured `herdr.control_command` (e.g. a lean
...
pub async fn herdr(
    ...
    // `build_commands` registers `/herdr` only when `herdr.control_command`
    ...
    let Some(command) = bot.config.herdr.control_command.clone() else {
        ctx.send(
    ...
    let socket = bot.config.herdr.socket_path();
    let extra_env = [
        ("HERDR_ENV", "1".to_owned()),
        ("HERDR_SOCKET_PATH", socket.to_string_lossy().into_owned()),
    ];
    let prompt = control::control_prompt(&prompt);
    let cwd = bot.config.herdr.control_cwd();
    let timeout = bot.config.herdr.control_timeout();
    let outcome = control::run_control_command(&command, &cwd, timeout, &prompt, &extra_env).await;
    let reply = match outcome {
        Ok(output) => {
            let reply = control::truncate_reply(&output, bot.config.discord.control_reply_limit);
